...
with gr.Blocks(css=css, theme=gr.themes.Base()) as app:
    with gr.Column(elem_classes="dark-theme"):
        gr.Markdown("# Scholar Copilot - Your Academic Writing Assistant", elem_classes="title")
        ...
        1. **Next-3-Sentence Suggestions**: Facilitates writing by predicting the next sentences with automatic retrieval and citation of relevant reference papers.
        2. **Citation Suggestions on Demand**: Provides precise, contextually appropriate paper citations whenever needed.
        3. **Full Section Auto-Completion**: Assists in brainstorming and drafting comprehensive paper content and structure.
        """
        gr.Markdown(intro_text, elem_classes="intro")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                with gr.Row(elem_classes="button-row"):
                    complete_btn = gr.Button("Complete 3 sentences", elem_classes="custom-button-green")
                    generate_btn = gr.Button("Generate to the end", elem_classes="custom-button-blue")
                    citation_btn = gr.Button("Insert citation", elem_classes="custom-button-orange")
                    clear_btn = gr.Button("Clear All", elem_classes="custom-button-red")

        # 引用建议区
        with gr.Row():
            citation_box = gr.Group(visible=True, elem_classes="citation-section")
            with citation_box:
                ...
                insert_citation_btn = gr.Button("Insert selected citations", elem_classes="custom-button-blue")

        with gr.Row(elem_classes="button-row"):
            download_history_btn = gr.Button("Download Citation History", elem_classes="custom-button-green")
