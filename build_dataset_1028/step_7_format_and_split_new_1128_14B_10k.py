...
def single_process_data(paper_dir_path, tokenizer, use_multi_cite=False):
    step_5_data_path = os.path.join(paper_dir_path, "step_5_1123_info.json")
    ...
    while curr_length < 10000:
        ...
        if next_seg_token_num > 10000:
            break
        if curr_length + next_seg_token_num < 10000:
    ...
    for sub_dir in os.listdir(input_dir):
        ...
        if os.path.isdir(os.path.join(input_dir, sub_dir)):
            for paper_dir in tqdm(os.listdir(os.path.join(input_dir, sub_dir))):
                ...
                result = single_process_data(paper_dir_path, tokenizer, True)
                total_data_num += 1
                if result is None:
                    continue
                with open(os.path.join(paper_dir_path, "step_7_1128_14B_10k_info_use_multi.json"), "w") as fo:
