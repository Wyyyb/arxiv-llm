...
def run_on_darth_server(input_dir, output_log_path, num_processes=8):
    ...
    for sub_dir in os.listdir(input_dir):
        ...
        if os.path.isdir(os.path.join(input_dir, sub_dir)):
            paper_dirs = os.listdir(os.path.join(input_dir, sub_dir))
            total += len([p for p in paper_dirs if p.startswith(sub_dir)])

            # 创建偏函数，固定除paper_dir外的参数
            ...
            # 使用tqdm显示进度
            list(tqdm(
                pool.imap_unordered(process_func, paper_dirs),
                total=len(paper_dirs),
                desc=f"Processing {sub_dir}"
            ))
    ...
    # 将manager.list转换为普通list
    output_log = {
        k: list(v) for k, v in shared_dict.items()
    }
